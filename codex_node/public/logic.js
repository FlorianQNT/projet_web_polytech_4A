// --- CONFIGURATION ---
// On pointe vers notre API Node locale (même domaine)
const API_URL = "/api"; 
// On pointe vers GitHub pour les images
...
let dataStore = { gods: {}, aspects: [], marteaux: [] };
let currentBuild = [];
...
async function fetchAllData() {
    try {
        // --- MODIFICATION ICI : Utilisation de API_URL au lieu de api.php ---
        const godPromises = gods.map(god => 
            fetch(`${API_URL}?categorie=${god}`).then(r => r.json()).then(d => ({ k: god, v: d }))
        );
        
        const aspectPromise = fetch(`${API_URL}?categorie=armes`).then(r => r.json());
        const hammerPromise = fetch(`${API_URL}?categorie=marteaux`).then(r => r.json());
        ...
        console.error(e);
        els.loader.innerText = "Erreur chargement (Le serveur Node est-il lancé ?)";
    }
}

// ... LE RESTE DU FICHIER RESTE IDENTIQUE À TON ANCIEN logic.js (initInterface, renderBoons, etc.) ...
// Copie-colle tout le reste de ton logic.js original à partir de la fonction initInterface() incluse.
// Assure-toi juste que la variable 'baseUrl' est bien celle définie tout en haut.
// Voici la suite pour que le fichier soit complet :
...
function onWeaponTypeChange(type) {
    ...
    filteredAspects.forEach(aspect => {
        ...
        // Note: Ici on utilise baseUrl qui pointe vers GitHub
